...
Gruplama ve sign otomasyonu Puppeteer ile gelen test tarayıcısını değil, Windows'ta
kurulu gerçek Google Chrome'u headful fakat minimize/off-screen arka plan modunda kullanır. Bu düzen headless tarayıcı parmak izi kaynaklı CloudFront engeli riskini azaltır. Tüm üst seviye sayfa açma/yenileme,
login gönderimi ve kritik API istekleri üç botun paylaştığı tek bir hız sınırı kuyruğundan geçer. Varsayılan güvenli
aralıklar şunlardır:

- sayfa ve kritik API istekleri arasında en az 15 saniye,
- sign hesapları ve paketleri arasında en az 15 saniye,
- CloudFront 403 görülürse yalnız hatayı alan botta yaklaşık 2, 4 ve 8
  dakikalık adaptif bekleme; sunucunun `Retry-After` talebi hariç en fazla 15 dakika.
...
korur, yalnız ilgili botun devre kesicisini çalıştırır ve kaldığı isteği yeniden dener. Üç kontrollü
denemeden sonra dış servis hâlâ engelliyse mevcut JSON durumunu bozmadan durur.

Alt sınırdan küçük eski ortam ayarları (`5 sn` hesap geçişi, `8 sn` ağ
aralığı veya yaklaşık `40 sn` 403 beklemesi gibi) worker açılışında otomatik
olarak güvenli değere yükseltilir. Sign sunucusunun geçici `ERR901` yanıtı
hesabı başarısız saymaz; `success` veya açık `already_signed` yanıtı gelene
kadar aynı oturum ortak kapıyla kontrollü biçimde denenir.

Worker yönetimi:

```powershell
node .\automation.js --worker-status
node .\automation.js --disable-worker account
...
Başlangıç sihirbazında onaylanan dinamik hesap planı
`pipeline-runtime/session-config.json` içine yazılır. Şifre düz metin olarak
yazılmaz; Windows DPAPI ile yalnızca aynı Windows kullanıcısının çözebileceği
`pipeline-runtime/credentials.dpapi.json` kaydında tutulur. Böylece manager
...
$env:LEGEND_ACCOUNT_COOLDOWN_MS = '15000'
$env:LEGEND_GROUP_COOLDOWN_MS = '15000'
$env:LEGEND_CLOUDFRONT_BACKOFF_MS = '120000'
$env:LEGEND_CLOUDFRONT_BACKOFF_MAX_MS = '900000'
$env:LEGEND_CLOUDFRONT_MAX_ATTEMPTS = '3'
node .\grupla.js
```
